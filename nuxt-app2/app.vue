<template>
    <main>
      <h1>Zoznam ľudí</h1>
      <section class="list">
        <section v-for="person of persons" v-bind:key="person" v-bind:class="{'card-container':true,'card':true,'adult': person.birthday >=18}">
		...
		-->
          <article class="info">
			...
			<div class="age"> <img src="images-removebg-preview.png">{{Year - person.birthday}} <span class="year-title">  born </span> </div>
            <div class="age">{{person.birthday}} <span class="year-title" >years</span>  </div>
          </article>
...
<style>
	...
	.card.adult {
		border-top-color: rgb(143, 109, 46);
	}
